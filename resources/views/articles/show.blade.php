				...
				<article>
					<header class="flex flex-col">
						<div class="flex items-center gap-3 text-xs font-semibold tracking-wide uppercase text-indigo-600">
							...
							@auth							
								<a href="{{ url('/articles'.'/'.$article->id.'/edit') }}">
									<span class="px-1.5 py-1 text-sm font-medium text-white bg-indigo-600 border border-slate-300 rounded-lg shadow-2xs hover:bg-indigo-500 active:bg-blue-100 transition cursor-pointer">
										Edit
									</span>
								</a>
							@endauth
						...
						<div class="mt-6 flex items-center gap-3 border-y border-neutral-200/80 py-4">
							<img src="/storage/{{ $article->authors_image }}" alt="authors image" class="h-10 w-10 rounded-full bg-neutral-100 object-cover">
							<div>
								<p class="text-sm font-semibold text-neutral-950">Written by {{ $article->authors_name }}</p>
								<p class="text-xs text-neutral-500">{{ $article->authors_profession }}</p>
							</div>
						</div>
					</header>
					...
					<div class="mt-10 max-w-none text-base text-neutral-800 sm:text-lg leading-relaxed space-y-6">
						...
						<blockquote class="border-l-4 border-indigo-600 bg-neutral-100/60 p-5 rounded-r-xl my-6 italic text-neutral-700">
					...
					<footer class="mt-16 rounded-2xl border border-neutral-200 bg-white p-6 sm:p-8">
						<div class="flex flex-col sm:flex-row items-start gap-4 sm:gap-6">
							<img src="/storage/{{ $article->authors_image }}"  alt="Sarah Jenkins" class="h-14 w-14 shrink-0 rounded-full object-cover bg-neutral-100">
							<div class="space-y-2">
							<h3 class="text-base font-bold text-neutral-950">About {{ $article->authors_name }}</h3>
							<p class="text-sm text-neutral-600 leading-relaxed">{{ $article->authors_name }} is a core software engineer who focuses on tooling pipelines, modular asset compilation engines, and interface performance patterns across modern application platforms.</p>
							<div class="pt-2 flex gap-4 text-xs font-semibold text-indigo-600">
								<a href="https://twitter.com" class="hover:underline" target="_blank">Follow on Twitter</a>
								<a href="https://github.com" class="hover:underline" target="_blank">View GitHub Profile</a>
							</div>
							</div>
						</div>
					</footer>
				...
				<div class="sticky top-24 rounded-2xl border border-neutral-200 bg-white p-6 shadow-xs">
					...
					<form class="mt-4 space-y-2" onsubmit="event.preventDefault();">
						<div>
							...
							<input id="email-address" type="email" required placeholder="you@mail.com" class="w-full rounded-lg border border-neutral-300 bg-neutral-50 px-3 py-2 text-sm text-neutral-950 placeholder-neutral-400 focus:border-indigo-600 focus:bg-white focus:ring-1 focus:ring-indigo-600 outline-none transition-all">
						</div>
						<button type="submit" class="w-full rounded-lg bg-indigo-600 px-3 py-2 text-center text-sm font-semibold text-white shadow-xs hover:bg-indigo-500 transition-colors cursor-pointer">
							Subscribe
						</button>
					</form>
				</div>
